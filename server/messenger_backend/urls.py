...
def response_not_found_handler(request, exception=None):
    return JsonResponse({"error": "The route is not defined"}, status=404)


handler404 = response_not_found_handler
...
urlpatterns = [
    ...
    path("api/onboarding", api.Onboarding.as_view()),
]
